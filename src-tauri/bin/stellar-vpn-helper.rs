...
fn build_script(remotes: Vec<(String, u16, String)>) -> Result<String, String> {
  ...
  for (host, port, proto) in remotes {
    ...
    if ips.is_empty() {
      ...
      any_allow = true;
      if tcp {
        s.push_str(&format!("add rule inet stellarkillswitch output tcp dport {port} accept\n"));
      } else {
        s.push_str(&format!("add rule inet stellarkillswitch output udp dport {port} accept\n"));
      }
      continue;
    }
