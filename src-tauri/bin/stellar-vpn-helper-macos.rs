...
enum Req {
  Connect { openvpn: String, config: String, auth: String },
  Disconnect,
  Subscribe,
  Status,
}
...
fn is_safe_temp_path(p: &str) -> bool {
  ...
  s.starts_with("/var/folders/") || s.starts_with("/tmp/stellar-vpn-desktop") || s.starts_with("/tmp/")
}
...
async fn run_openvpn(
    ...
    tokio::spawn(async move {
      ...
      while let Ok(Some(line)) = r.next_line().await {
        ...
        if line.contains("Initialization Sequence Completed") {
          ...
          send_event(&tx, Event::Status { status: "connected".into() }).await;
        }
      }
    });
  }
  ...
  // child watcher
  {
    ...
    tokio::spawn(async move {
      ...
      send_event(&tx, Event::Log { line: format!("[mac-helper] OpenVPN exited (code={code})") }).await;
      ...
      send_event(&tx, Event::Status { status: "disconnected".into() }).await;
    });
  }
  ...
  match req {
    Req::Subscribe => {
      ...
      loop {
        match rx.recv().await {
          Ok(msg) => {
            if reader.get_mut().write_all(format!("{msg}\n").as_bytes()).await.is_err() {
              break;
            }
          }
    ...
    Req::Disconnect => {
      ...
      send_event(&ev_tx, Event::Status { status: "disconnected".into() }).await;

      let _ = reader
        .get_mut()
        .write_all(serde_json::to_string(&Resp { ok: true, error: None, status: None }).unwrap().as_bytes())
        .await;
      let _ = reader.get_mut().write_all(b"\n").await;
    }

    Req::Connect { openvpn, config, auth } => {
      // Hardening
      ...
      if !Path::new(&config).exists() || !is_safe_temp_path(&config) && !Path::new(&config).is_file() {
      ...
      send_event(&ev_tx, Event::Status { status: "connecting".into() }).await;
      send_event(&ev_tx, Event::Log { line: "[mac-helper] starting OpenVPN…".into() }).await;
